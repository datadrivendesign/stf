...
module.exports = function(options) {
  ...
  app.post('/app/api/v1/noauth/responses/:token', function(req, res) {
    ...
    dbapi.getRedactMode(token).then(function(token_obj) {
      if (token_obj[0].task.willRedact) {
        return res.redirect(`${config.redactionUrl}?token=${token}`);
      } else {
        return res.redirect('/auth/token/' + token);
      }
    }).catch(function(err) {
      console.error('Error getting redact mode', err);
      res.sendStatus(500);
    });
  });
