...
var receiveEvent = require('../tapp/ui-data-interceptor').receiveEvent;
...
module.exports = function(options) {
  ...
  io.on('connection', function(socket) {
    ...
    // We add an extra event parameter.
    function createKeyHandler(Klass, evt) {
      return function(channel, data) {
        receiveEvent(evt, data, function () {
          push.send([
            channel
    ...
    var messageListener = wirerouter()
      ...
      .on(wire.JoinGroupMessage, function(channel, message) {
        // console.log('ERIK: device.change, applyOwner', message.serial, message.owner)
        socket.emit('device.change', {
          important: true
    ...
    new Promise(function(resolve) {
      socket.on('disconnect', resolve)
        ...
        .on('input.touchDown', function(channel, data) {
          receiveEvent('input.touchDown', data, function () {
            push.send([
              channel
        ...
        .on('input.touchMove', function(channel, data) {
          receiveEvent('input.touchMove', data, function () {
            push.send([
              channel
        ...
        .on('input.touchUp', function(channel, data) {
          receiveEvent('input.touchUp', data, function () {
            push.send([
              channel
        ...
        .on('input.touchCommit', function(channel, data) {
          receiveEvent('input.touchCommit', data, function () {
            push.send([
              channel
        ...
        .on('input.touchReset', function(channel, data) {
          receiveEvent('input.touchReset', data, function () {
            push.send([
              channel
        ...
        .on('input.gestureStart', function(channel, data) {
          receiveEvent('input.gestureStart', data, function () {
            push.send([
              channel
        ...
        .on('input.gestureStop', function(channel, data) {
          receiveEvent('input.gestureStop', data, function () {
            push.send([
              channel
        ...
        // Key events
        .on('input.keyDown', createKeyHandler(wire.KeyDownMessage, 'input.keyDown'))
        .on('input.keyUp', createKeyHandler(wire.KeyUpMessage, 'input.keyUp'))
        .on('input.keyPress', createKeyHandler(wire.KeyPressMessage, 'input.keyPress'))
        .on('input.type', function(channel, data) {
          receiveEvent('input.type', data, function () {
            push.send([
              channel
        ...
        .on('display.rotate', function(channel, data) {
          receiveEvent('display.rotate', data, function () {
            push.send([
              channel
        ...
        .on('clipboard.paste', function(channel, responseChannel, data) {
          receiveEvent('clipboard.paste', data, function () {
        ...
        .on('clipboard.copy', function(channel, responseChannel) {
          receiveEvent('clipboard.copy', data, function () {
            joinChannel(responseChannel)
            push.send([
              channel
            , wireutil.transaction(
