      ...
      socket.on('disconnect', resolve)
        ...
        .on('user.keys.accessToken.remove', function(data) {
          return dbapi.removeUserAccessToken(user.email, data.title)
            .then(function() {
              socket.emit('user.keys.accessToken.removed', cryptutil.encrypt(data.title))
            })
        })
        ...
        .on('input.gestureStart', function(channel, data) {
          ...
          layoutCaptureService.enqueue(wire.GestureStartMessage, function() {
            ...
            var imgId =  data.imgId.split('_')[1];
            viewResHandlers[data.serial + imgId] = function(viewHierarchy) {
            ...
            setTimeout(function noResponse(){
              if (viewResHandlers[data.serial + imgId]) {
                log.warn('View hierarchy response timed out, ' +
                  'skipping request. --------------------------------------  ' +
                  data.serial + ":" + imgId);
                var viewResHandler = viewResHandlers[data.serial + imgId];
                viewResHandlers[data.serial + imgId] = null;
        ...
        .on('input.replay', function(channel, responseChannel, data) {
          ...
          push.send([
            ...
            , wireutil.transaction(
              responseChannel
              , new wire.ReplayMessage(data.token)
            )
          ]);
        })
