...
var websocketModule = require('../../../websocket');
var frameInterceptor = require('../../../tapp/frame-interceptor')

module.exports = syrup.serial()
  // .dependency(require('../../../tapp'))
  .dependency(require('../../support/adb'))
  .dependency(require('../../resources/minicap'))
  .dependency(require('../util/display'))
  .dependency(require('./options'))
  .define(function(options, /* TappInterceptor, */ adb, minicap, display, screenOptions) {
    ...
    return createServer()
      .then(function(wss) {
        ...
        wss.on('connection', function(ws) {
          ...
          function wsFrameNotifier(frame, sessionId) {
            return new Promise(function(resolve, reject) {
              switch (ws.readyState) {
              case WebSocket.OPENING:
                // This should never happen.
              ...
              case WebSocket.OPEN:
                // Erik: Intercept the frame, tied to the sessionId.
                frameInterceptor.receiveFrame(frame, sessionId);
                var frameCount = frameInterceptor.getImgCount(sessionId);
          ...
          // Takes a sessionId and returns a bound version of wsFrameNotifier.
          function wsFrameNotifierBound (sessionId) {
            return function (frame) {
              // console.log('CALLED WITH', sessionId)
              wsFrameNotifier(frame, sessionId);
            }
          }
          ...
          ws.on('message', function(data) {
            ...
            if (data.indexOf('size ') === 0){
              ...
              sessionId = data.slice(4);
              broadcastSet.remove(id)
              
            } else if (data.indexOf('off') === 0) {
              broadcastSet.remove(id)
              // TODO: Stop streaming frames to fs for sessionId
            }
          })
