...
const FRAME_DELAY = globalConfig.throttlingTimeoutMs;
...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, minicap, display, screenOptions) {
    ...
    function FrameProducer(config) {
      ...
      const deviceInfo = globalConfig.deviceInfo[options.serial] || globalConfig.deviceInfo.default
      this.deviceCaptureWidth = deviceInfo.resolutionX * globalConfig.minicapCompressionFactor
      this.deviceCaptureHeight = deviceInfo.resolutionY * globalConfig.minicapCompressionFactor
      this.compressionWidth = deviceInfo.resolutionX * globalConfig.clientCompressionFactor
      this.compressionHeight = deviceInfo.resolutionY * globalConfig.clientCompressionFactor
      log.info(this.deviceCaptureHeight)
      log.info(this.deviceCaptureWidth)
      log.info(this.compressionHeight)
      log.info(this.compressionWidth)
    }
    ...
    FrameProducer.prototype.setStaticProjection = function() {
      if (this.frameConfig.virtualWidth === this.deviceCaptureWidth &&
          this.frameConfig.virtualHeight === this.deviceCaptureHeight) {
        log.info('Keeping %dx%d as current frame producer projection',
          this.deviceCaptureWidth, this.deviceCaptureHeight)
        return
      }

      log.info('Setting frame producer projection to %dx%d',
        this.deviceCaptureWidth,
        this.deviceCaptureHeight)
      this.frameConfig.virtualWidth = this.deviceCaptureWidth
      this.frameConfig.virtualHeight = this.deviceCaptureHeight
      this._configChanged()
    }
    ...
    return createServer()
      .then(function(wss) {
        ...
        wss.on('connection', function(ws) {
          ...
          function sendFrame(frame, id, curDate, reject, resolve) {
            prevDate = curDate;

            var fileName = frameStore.getNextFileName(id);
            var message = util.format(
                  'nextImgId %s'
                  , fileName
            )

            frameStore.storeFrame(frame, fileName);
            ...
            // This is what SHOULD happen.
            // First resize the image with sharp, then sends the image through websockets
            sharp(frame).resize(this.compressionWidth, this.compressionHeight, {kernel: sharp.kernel.cubic}).sharpen()
          ...
          function wsFrameNotifier(frame, id) {
            return new Promise(function(resolve, reject) {
              ...
              case WebSocket.OPEN:
                ...
                frameCount += 1;
                if (curDate - prevDate > FRAME_DELAY) {
                  sendFrame(frame, id, curDate, reject, resolve);
                } else {
                    dropCount += 1;
                    if (newFrame) {
                      newFrame = false; // Needs a timestamp solution
                      setTimeout(function noNewFrame() {
                        if (!newFrame && curDate > prevDate) {
                          sendFrame(frame, id, curDate, reject, resolve);
                          dropCount -= 1;
                          prevFrame = null;
                        }
                      ...
                      // Sets a time out for the first frame that is within the throttling timeout
                      // after previous frame delivery, this ensures delivery of last stable frame even if
                      // it's delivered within the timeout of the second last stable framframe
                    }};
                return
              case WebSocket.CLOSING:
                // Ok, a 'close' event should remove the client from the set
