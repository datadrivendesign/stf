...
module.exports = function(options) {
  ...
  var basicAuthMiddleware = function(req, res, next) {
    ...
    if (
      user.name === options.mock.basicAuth.username &&
  ...
  var resetSession = function(req, res, next, tokenObj) {
    var log = logger.createLogger('auth-token')
    log.info('Token Object "%s"', JSON.stringify(tokenObj))
    ...
    if (tokenObj) {
      if (tokenObj.isReplayToken || tokenObj.isDesignerToken) {
        res.render('designer-taskend')
      }
 else if (tokenObj.upworkUser) {
        dbapi.hasFeedback(tokenObj.token).then(function(submittedFeedback) {
          res.render('upwork-taskend', {
        ...
        dbapi.hasResponse(tokenObj.token).then(function(submittedTask) {
          res.render('turk-taskend', {
  ...
  app.get('/auth/token/', resetSession)
  app.get('/auth/token/:token', function(req, res, next) {
    res.clearCookie('XSRF-TOKEN')
    res.clearCookie('ssid')
    res.clearCookie('ssid.sig')
      ...
      dbapi
        ...
        .then(function(tokenObj) {
          if (tokenObj && tokenObj.status === 'unused') {
            ...
            setTimeout(function() {
              console.log('Kicking user, reached timeout for token', tokenObj.token)
              request.delete(
                options.appUrl + '/app/api/v1/token/' + tokenObj.token + '?authed=true',
                function(err, res) {
                  if (err) {
                    log.error('Error kicking user token ', tokenObj.token, err.stack)
                  }
                }
              )
            ...
            return resetSession(req, res, next, tokenObj)
          }
 else {
  ...
  app.get('/auth/static-task', function(req, res) {
    res.clearCookie('XSRF-TOKEN')
    res.clearCookie('ssid')
    res.clearCookie('ssid.sig')

    res.render('static-task', {
      submitUrl: options.appUrl + 'auth/static-task/submitted'
    })
  })

  app.get('/auth/static-task/submitted', function(req, res) {
    res.render('turk-taskend', {
      submittedTask: true
      , contactEmail: config.contactEmail
    })
  ...
  app.get('/auth/task-error', function(req, res) {
